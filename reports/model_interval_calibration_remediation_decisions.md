...
shipped model (SARIMA, SARIMAX Group D, Elastic Net); three separately-motivated
remediation attempts were rejected because each measurably hurt real
walk-forward RMSE and/or coverage despite being individually well-motivated.
SARIMAX was later removed from the project entirely (2026-08-26); the findings
below are the evidence base for that decision.
...
(`cross_val_predict` can't be used directly here: it requires every row to
...
Elastic Net/ensemble's under-coverage. Fresh calibrated coverage: `sarima`
70.8%, `ensemble` 65.8%, `elastic_net` 64.4%.
...
improvement, but used a stale pre-refit baseline; recomputed properly on the
current dataset:
...
Feature pruning did not fix it (instability came from the `(1,0,1)` order
...
5%), this is part of the evidence for removing SARIMAX entirely.
...
transform per walk-forward window only, fit the same SARIMA order on the
transformed series, and inverse-transformed simulated draws back (avoiding
Jensen's-inequality bias by Monte-Carlo-averaging rather than correcting
analytically).
...
made the *model's residuals* less normal, evidence that the fat tails come
from genuine shock quarters (COVID, 2021-22 energy/inflation surge), not a
smooth, removable scale effect. Not adopted.

## Why All Three Attempts Failed The Same Way

Classical assumptions (normality, comfortably-stationary roots) exist for
inference validity, not forecast accuracy, and satisfying them is not
guaranteed to improve either:

- Near-unit-root AR behaviour likely reflects genuine inflation persistence,
  not a numerical artifact. Forcing comfortably-stationary roots implicitly
  asserts faster mean-reversion than the data shows, a real bias.
...
- AIC/Jarque-Bera/Ljung-Box are in-sample diagnostics; walk-forward RMSE and
  coverage are genuinely out-of-sample. With ~100-125 observations these can
  diverge, and the best in-sample fit can be overfit to that sample's
  idiosyncrasies.
...
single-equation SARIMAX ever did at this sample size.
...
- Trimmed-mean SARIMA: heteroskedastic residuals (breakvar p=0.005). Not
  ...
  ~124-quarter sample, and the pattern above gives no reason to expect it
  would survive walk-forward validation.
...
symmetric. For skewed paths, `scale_factor=1.0` was not an identity
operation, making calibrated coverage worse than raw for several
model/horizon combinations (all 8 trimmed-mean Elastic Net horizons; 6 of 8
headline SARIMA).
...
score. A larger/rolling calibration window and a genuinely asymmetric score
were left as future work, implemented in the next section.
...
4. *Reporting mismatch.* The trimmed-mean coverage report was written
   uncalibrated while headline's was calibrated.
...
target quarters, floored at 1.0; the same logic for the live "current"
factor the API serves; and the API now scales each side around the point to
match what the coverage report evaluates.
...
post-2020 coverage falls as the window lengthens. 12 quarters was kept as a
middle choice, as good as longer windows on post-2020 coverage, avoids the
...
- Coverage is still below 80% after 2020 for every family. No interval built
  ...
  follows it with a lag, and pays for it in width (held-out widths roughly
...
- The factor is pooled across horizons, so short-horizon intervals are
  ...
  fixes trimmed mean's normal-period h1-2 over-coverage but under-covers
  long horizons and headline, so the simpler pooled design was kept.
...
- The window was chosen on quiet-period data containing no volatility shock,
  so the choice among windows is weakly identified; the post-2020 numbers
  are out-of-sample for it but are one episode, not a distribution.
