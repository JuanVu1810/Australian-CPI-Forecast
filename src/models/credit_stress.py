...
CREDIT_STRESS_CAVEAT = (
    "Credit-stress PD base values are illustrative assumptions, not calibrated "
    "to any real portfolio, including NAB's. The unemployment-sensitivity "
    "coefficients, 0.4 for personal loans and 0.6 for mortgages, are the RBA's "
    "published values from Garvin et al (2022), RBA Research Discussion Paper "
    "No 2022-03, applied uniformly across the nine banks covered in that paper "
    "(including NAB), not adapted or derived for any specific institution. The "
    "mortgage coefficient is applied in a simplified form without the paper's "
    "LVR-bucket tiering, and the floor used here is the segment's own PD_base "
    "rather than the paper's separate lower natural-default-rate floor PD-bar, "
    "which this project does not model as a distinct parameter. This is an "
    "illustrative demonstration of a documented "
    "industry stress-testing pattern, not a validated credit risk model, and "
    "must not be used for actual credit or regulatory decisions. For context, "
    "APRA's aggregate non-performing/impaired-loan ratio is available in this "
    "repo at data/curated/credit_quality_quarterly.csv as corroborating context "
    "only; it does not calibrate the PD base values or sensitivity coefficients."
)
